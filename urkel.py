...
def _initialize(bot):
    ...
    plugins.register_user_command(["latest"])
...
def latest(bot, event, user):
    ...
    try:
        ...
        heading = "<b>{}'s Recent Tracks</b>\n\n".format(user.capitalize())
        yield from bot.coro_send_message(event.conv_id, heading)
        ...
        try:
            for i in range(0, 10):
                top_10 = top_10 + "<b>" + str(i + 1) + "</b>. " + recent_tracks[i]["artist"] + " - " + recent_tracks[i]["track"] + "\n"
